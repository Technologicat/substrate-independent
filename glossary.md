...
**Connections:** Orthogonal to the growth patterns — *[snowball effect](#snowball-effect)*, *[hydra tax](#hydra-tax)*, *[polish cascade](#polish-cascade)* — which name what accumulates; continuation names how the path is steered, each step fixed by the last rather than by the start. Any of them can proceed this way, and then drifts as well as grows. The case continuation contributes alone is the one where nothing grows at all: nothing got bigger, the subject moved. The determinism is local and the freedom is in the path (which associative thread is followed), so the same opening may reach a different destination by a different route.  
...
**Context:** *"I tried a coding agent, but every time I started it, it was all like 'Please state the nature of your programming emergency.' How come yours doesn't do that?"*
...
**Context:** Scoping decisions about which edge cases a feature handles, where the answer is set by what the rest of the codebase already handles.
...
**Meaning:** A strategy for high-quality software, honest about what it costs. The first attempt at any non-trivial design will get something wrong, often in ways only visible after living in the result. Build it. Rebuild the parts that strained, informed by the first attempt; then again, informed by the first two. The count is nominal and the unit is the part rather than the whole — a component, or one mechanism iterated until it no longer resembles the early attempts. The final is then either assembled from the parts that survived, or is simply the last iteration, once it is what was wanted. Three builds is genuinely more expensive than one, and historically the cost has been the bottleneck — the activation energy for *rebuild the part that didn't work* was higher than the available budget on most projects most of the time, leaving build-once-and-live-with-it as the de facto alternative. AI-assisted development lowers the cost of each build far enough that the rebuilds, previously priced out, now fit. Deciding what survived, or when to stop, still requires a human evaluator: hearing which parts came out *[aria-worthy](#aria-worthy-design)* and which are still off-key.  
**Etymology:** Inverts the carpenter's *measure twice, cut once*, whose economics assume the cut is the expensive step; here building is cheap enough to serve as the measurement. The lineage runs through Brooks's *The Mythical Man-Month* (1975): *plan to throw one away; you will, anyhow* — a maxim Brooks's own twentieth-anniversary edition (1995) called too simplistic, preferring incremental refinement. Between them, the two editions name both ways of finishing.  
...
## Chekhov's landmine
**Meaning:** An incidental specific — a count, a version, a model name, a "there is no way to…" — that is true when written and decays silently, detonating later under a reader who has no reason to doubt it. Chekhov's gun is planted deliberately and must go off; this one is planted absent-mindedly, goes off anyway, and points the wrong way.  
**Nuance:** A *dated record* is not one. A measurement in an investigation, a testing history, a stored sample naming what produced it — those are pinned to their moment on purpose, and the pinning is what makes them true. The landmine is a decaying claim wearing the clothes of a current one; the tell is a specific that nothing will ever recheck.  
**Connections:** Shares its economics with *[depleted uranium disclaimer](#depleted-uranium-disclaimer)* — free to write, reads as diligence, accumulates — but the failure differs: that one guards a risk that was never live, this one is correct on the day and false by the time it matters. The sibling on disk is *[precedent poisoning](#precedent-poisoning)*, where the text was never decided rather than once true. *[Version pinning](#version-pinning)* guards against the opposite drift — a reference whose referent moves while the text stays valid — with the same repair: pinned to its date, a landmine becomes a dated record.  
**Context:** Documentation and changelogs; anywhere prose states a number the code can change without telling it.

## Closing bell
**Meaning:** An AI agent's tendency to bid for session termination at every natural boundary — after each commit, each milestone, each resolved subtask — even when significant follow-on work is visible. The pattern is structurally interesting: the agent models the session as a sequence of discrete tasks with clean endpoints, and at each endpoint, checks whether the human wants to stop. But the most valuable work often happens in the follow-on — the human who says "actually, while we're here..." is the one who turns a version number declared in three places into a single source of truth, or who discovers a stale CI badge that's been wrong since the workflow was first set up.  
...
## Concurrent ravioli
**Meaning:** Of a codebase: concurrency-relevant operations spread thinly across many small, individually well-encapsulated components, such that a textbook-level concurrency error is invisible at every single site. No one file shows the mistake, because no one file contains it — the fault is in the composition, and the composition is what the encapsulation exists to hide. The characteristic aftertaste is *"how did that survive review?"*, and the answer is that every review looked at a unit, and every unit was fine. The safety mechanisms work at the same grain, so they can make it worse: a guard that makes each unit safe to write can turn the composition's failure from a crash into silence.  
**Etymology:** From *ravioli code*, the counterpart to *spaghetti code*: where spaghetti tangles control flow, ravioli parcels it into so many tidy packets that following it means opening all of them. "Concurrent" names the axis on which the parcelling does real damage rather than merely costing patience.  
**Connections:** Of the genus of competent-looking failures, with *[confabulation](#confabulation)* and *[microwaving the soft drink](#microwaving-the-soft-drink)* — but here the competence is the *code's* rather than the reader's or the model's: every piece is right and the assembly is not. The distance involved is the one *[spooky action at a distance](#spooky-action-at-a-distance)* names.  
**Context:** Dispatch loops over a registry whose members can reach back into it — listeners that unsubscribe during notification, timers that cancel timers, animations that cancel animations. A reentrant lock makes a good accomplice: it lets the callback back in, where a plain lock would have deadlocked on the spot.
...
**Meaning:** A development posture in which a very small team builds bespoke load-bearing components for its own system, each of which would previously have been a project in itself — a GPU-accelerated resampler written for one application's needs, a graph-viewer widget rebuilt from scratch against a different toolkit — because AI-accelerated development has brought them within the budget of a subtask. The characteristic move is reaching one step past what the available parts make *easy*, into what they make *possible*.  
**Connections:** The economics are 🌐[W. Brian Arthur](https://en.wikipedia.org/wiki/W._Brian_Arthur)'s (*The Nature of Technology*, 2009): technologies evolve by combining existing ones, so what is buildable is a moving frontier defined by what already exists to be combined. Cyberpunk engineering is a small team occupying the position on that frontier — one combination out from the current stock — that corporate R&D used to hold alone. Distinct from Kauffman's *adjacent possible*, which names the frontier; this names who is standing on it and how they got there. Downstream problem: the practice reliably produces generally-useful components trapped inside one application's repository, which is what an *[upstream lift](#upstream-lift)* is for.  
**Etymology:** *Cyberpunk* from the literary genre, for its founding premise that powerful technology ends up in individual hands rather than only institutional ones. The genre supplies the posture, not the aesthetic. *Engineering* because the output has to hold weight afterwards.  
**Context:** One-person and two-person projects in the AI-assisted era. The constraint on *how much this many people can build* has moved, and so has the constraint on how much they can maintain: dormant projects come back into active upkeep, because the mechanical half of maintenance got cheap. What has not scaled with either is review. The human remains the single point everything passes through, so the binding constraint moves from *writing* to *deciding*, which is a better place for it to sit but is still a ceiling. What it caps is attention, which can be divided between the number of projects and the ambition of each.
...
**Connections:** Cousin to 🌐[security theater](https://en.wikipedia.org/wiki/Security_theater) (Schneier) — both perform a posture in place of work, but security theater defends against a real-but-mishandled threat, whereas a depleted uranium disclaimer performs diligence against a threat that was never live.  
...
**Context:** Long-lived public APIs; see [*The Discordian Deprecation*](field-observations.md#the-discordian-deprecation).
...
**Nuance:** The success rate is conditional on the perceiver: the prior is compiled experience, learned from cases rather than from rules, which is why it resists naming. It is also domain-bound — the confidence transfers to adjacent fields where the pattern base does not. The forward-looking claim is about direction, not implementation — *[build thrice, cherrypick](#build-thrice-cherrypick)* still applies to the code. The Lisp maxim that the code *wants to be written this way* demands the implementation be right the first time; jnsq claims only the direction.  
...
## Jukebox random
**Origin:** Heard from a coworker in the software industry, early 2000s, in a discussion of automatic playlist filling; assumed at the time to be in wide use, never looked up. The name carries the implication that a properly built jukebox has always worked this way.  
...
## Kolmogorov taming
**Meaning:** The way nontrivial UX and GUI work actually proceeds: one small detail at a time, with no pass that gets them all at once. Such code carries many details, none derivable from the others, so there is nothing to compress and no abstraction that retires a class of them. Some interfere with each other — fixing one exposes or masks another — and have to be considered together; the rest are independent, each observed and fixed on its own. Progress is therefore counted in details rather than in features, and the count is not knowable in advance. Hence the schedule: a week or two for anything nontrivial, reserved with a load-bearing sigh, by anyone who has done it before.  
**Etymology:** From 🌐[Kolmogorov complexity](https://en.wikipedia.org/wiki/Kolmogorov_complexity), the length of the shortest program that produces a given object.  
**Nuance:** Distinct from *[polish cascade](#polish-cascade)*, which it closely resembles. The cascade is a chain: each fix makes the next item visible, by making it look wrong next to the thing just fixed. Taming is a heap. Polish in a dense enough domain is taming too.  
...
**Context:** UX and GUI work, and the estimates made for it.
...
**Connections:** The complement of the *[closing bell](#closing-bell)*: the agent bidding to stop at each local boundary is, by the same locality, the one that cannot glance sideways. A frequent target of the glance is a *[snowball effect](#snowball-effect)* in the making — cross-project coupling a session can't see coming.  
...
## Polish cascade
**Meaning:** In programming, the phenomenon where a small fix reveals an adjacent inconsistency, which once fixed reveals another, and so on — each individually trivial, collectively unstoppable. Produces a chain of atomic commits whose git log reads like someone slowly realizing they should have grepped first. The cascade is not really about missing a search; each fix shifts what you *notice*, and you couldn't have grepped for "things that will look wrong once you've fixed this other thing." Not to be confused with anything from Warsaw. See also: *steering tax*.  
**Connections:** *[Hammerspace](#hammerspace)* names where any single item in the cascade comes from; the cascade is one of the shapes a sequence of them takes. *[Kolmogorov taming](#kolmogorov-taming)* is the heap to its chain. When each fix sets the standard for the next, the cascade drifts as well as grows — *[analytic continuation](#analytic-continuation)*, with consistency as the germ.  
**Context:** Code review and polish passes in AI-assisted development; the gravitational pull of local consistency.

## Precedent poisoning
**Meaning:** A failure mode of anything committed: an approach, idiom or turn of phrase spreading through a project because it was written down rather than because anyone chose it. Committed material reads as sanction — an ad-hoc spelling in the tree does not say *someone was in a hurry here*, it says *this is how it is done in this project*, and the next thing needing that job copies it. So an incorrect spelling does not sit there being merely wrong; it propagates, carrying the authority of everything around it that was done right.  
**Nuance:** *[Context corruption](#context-corruption)* for approaches rather than facts, and on disk rather than in a conversation, so it outlives the session that introduced it and the next one reads it as precedent. It hits hardest where it meets a coding agent's ingrained habits, the ones that instructions only partly hold back: a committed instance of such a habit is an example in the tree siding with the prior. The agent starts behaving subtly worse, with nothing obvious to point at. A diff shows the line, but not that it was a lapse, nor that later behaviour traces back to it. There is no cheap remedy but eternal vigilance.  
**Connections:** The mechanism of *[prose basilisks](#prose-basilisks)* one level over — memetic reproduction of a form — with a repository in place of a corpus. Shares a failure surface with *[Chekhov's landmine](#chekhovs-landmine)*: both are text on disk read with more authority than it earned, one because it was true once, the other because it was never decided.  
**Context:** Code review; documentation; why house style is worth enforcing on small things.

## Prequisite
**Part of speech:** n.  
**Meaning:** A unit of work emitted *toward* completing the current task, structurally symmetric to a follow-up. Where a follow-up is work emitted *by* completing something — discovered as a consequence and bumped to its own ticket — a prequisite is work that belongs before it: an architectural cleanup whose absence is the reason the current task is harder than it should be, surfaced when designing the current task and likewise bumped to its own ticket. Both are projects in their own right, with their own PRs; the relationship is sequential, not hierarchical.  
**Nuance:** A *prerequisite* blocks: the task cannot proceed until it is cleared. A *prequisite* does not — the current task can be completed without it, worse. Taking the option yields the better design. A prerequisite is dependency-as-obstacle; a prequisite, dependency-as-companion. Both prequisite and follow-up are intra-project; the cross-project analog is *[upstream lift](#upstream-lift)*.  
...
**Context:** Feature work where the same people maintain the upstream libraries, so revealed coupling gets fixed at its source rather than worked around. A tooltip showing per-node data from a graph needs the graph generator to emit that data — an upstream release, with internal improvements on the way through — and the viewer code built up around it starts wanting to be a library of its own.

---

## Spooky action at a distance
**Origin:** Einstein's objection to quantum-mechanical nonlocality — *spukhafte Fernwirkung*, from his 1947 letter to Max Born, and the informal name for the discomfort argued formally in the 🌐[EPR paper](https://en.wikipedia.org/wiki/Einstein%E2%80%93Podolsky%E2%80%93Rosen_paradox) (1935). Long since borrowed into programming, where it names an antipattern rather than a physical claim; this entry reports the programming sense, for 🌐[today's lucky 10,000](https://xkcd.com/1053/).  
**Meaning:** An effect whose cause is nowhere near it: the code in front of you behaves as it does because of something written somewhere you had no reason to look. What makes it *spooky* rather than merely indirect is that the local reading is **correct and insufficient** at once — nothing at the site is wrong, and nothing at the site is enough. The reader therefore has no signal that anything is missing; ordinary complexity at least announces itself.  
**Connections:** In OOP the leading source is inheritance of *concrete* classes, each level piling a little behaviour onto its ancestor, so that what a call means depends on a chain the call site does not name — 🌐[Julia](https://julialang.org/)'s answer being that only abstract types may be subtyped at all, which removes the mechanism instead of warning about it. In concurrency the distance is temporal rather than lexical: the cause is not elsewhere in the text but elsewhere in *time*, on another thread. Reentrancy gives the same distance without a second thread, the cause sitting elsewhere in the call stack. *[Concurrent ravioli](#concurrent-ravioli)* is either kind spread thin enough that no site shows it.  
**Context:** Global mutable state, monkeypatching, import-time side effects, configuration read from the environment, the CSS cascade, and lint suppressions scattered through files rather than stated once in the policy.
...
## Version pinning
**Original meaning:** In package management: fixing a dependency to an exact version rather than to a range or a floating tag, so that what gets installed is decided by whoever wrote it down and not by whoever released something that morning. In the GitHub Actions form the pin is a full commit SHA, conventionally trailed by a `# v6.0.3` comment — the SHA is what the machine resolves and what cannot be silently repointed, the comment is what a human reads, and the practice keeps both because neither does the other's job. Pinning does not mean freezing, which is the usual first objection: Dependabot reads the comment as the version anchor and bumps SHA and comment together, so updates keep arriving — as reviewable pull requests rather than as a change to what today's build resolves to.  
**Extended meaning:** The same discipline applied to any reference expected to outlive its writing, on the grounds that the failure it guards against is the same in all of them and unusually hard to see. An unpinned reference does not break loudly — its *syntax* stays valid while its *referent* moves. A workflow pinned to `@v6` still resolves; a document saying "what we ship today" still parses; a source comment citing `app.py:2978` is still a number. The pointer reports success while pointing somewhere other than it did, so the artifact reads as correct to everyone who checks it, including whoever wrote it. The pinned counterparts, respectively: the commit hash, the version number or the date, the named symbol.  
**Etymology:** The pin is the dressmaker's rather than the wrestler's: it holds a thing where you put it.  
**Connections:** The inverse failure is *[Chekhov's landmine](#chekhovs-landmine)*: a specific that stays put while the world moves on.  
**Context:** Supply-chain hardening, after the 🌐[tj-actions/changed-files compromise](https://www.stepsecurity.io/blog/harden-runner-detection-tj-actions-changed-files-action-is-compromised) made the case for SHA-pinning; documentation rot; and measurement records, where the deictic costs most, since being a fixed baseline is the entire value of the artifact.
...
*Started: 2026-02-05. Last updated: 2026-09-22.*
