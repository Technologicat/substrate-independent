...
**Connections:** Shares its economics with *[depleted uranium disclaimer](#depleted-uranium-disclaimer)* — free to write, reads as diligence, accumulates — but the failure differs: that one guards a risk that was never live, this one is correct on the day and false by the time it matters.  
**Context:** Documentation and changelogs; anywhere prose states a number the code can change without telling it. Coined 2026-09-21 during a Raven documentation pass that defused four in an afternoon: "what the GUI receives as *fifteen* events", "*December 2025*: the current recommendation is…" still standing ten months on, "there is *no way* to switch the audit off" written while the flag to do so existed, and a keyboard said to move between three panes a release after it became four.
...
**Connections:** Of the genus of competent-looking failures, with *[confabulation](#confabulation)* and *[microwaving the soft drink](#microwaving-the-soft-drink)* — but here the competence is the *code's* rather than the reader's or the model's: every piece is right and the assembly is not.  
**Context:** Raven, 2026-09-10. `Animator.render_frame` walked its registry directly, while an animation four frames down its own call chain cancelled a different one — shifting the list under an index-based iterator, so whichever animation sat next to the removed one was skipped and silently dropped. The mutation and the iteration were eighty lines apart in one file, connected only through a call chain crossing three classes. The reentrant lock is what kept it quiet: a plain `Lock` would have deadlocked at the cancel, loudly, the first time any view scrolled to its end — so the construct that made each unit safe to write is what let their composition fail in silence.
...
**Connections:** The complement of the *[closing bell](#closing-bell)*: the agent bidding to stop at each local boundary is, by the same locality, the one that cannot glance sideways. A frequent target of the glance is an *[snowball effect](#snowball-effect)* in the making — cross-project coupling a session can't see coming.  
...
## Prose poisoning
**Origin:** Juha, 2026-09-11, naming what had just happened in the session that was running.  
**Meaning:** *[Context corruption](#context-corruption)*, but for coding idioms, patterns and approaches rather than for facts. A spelling written into a codebase becomes a template, and the next thing needing that job copies whichever one it happens to read — so an ad-hoc idiom does not merely sit there being worse than the house one, it *propagates*. The cost is not the one bad line; it is that every later reader now has two spellings for one job and no way to tell which was a decision and which an accident.  
**Why it spreads, as against merely sitting there:** committed code reads as *sanction*. A `SENTINEL = object()` in the tree does not say "someone was in a hurry here"; it says *this is how sentinels are done in this project*, and the next person needing one has been shown the answer. So the lapse is not local — it is an invitation, and it is accepted by readers who have no way to know it was never decided.  
**Live case:** Raven's `configoverrides` used `unpythonic.sym` for a sentinel, and its own test, written twenty minutes later in the same changeset, used an `enum.Enum` wrapping a bare `object()`. Two idioms for one job, introduced together, by the same author, in one diff — and the ad-hoc one was the more imitable, being the one that needs no import. What it costs against `sym` is small and real: a readable `repr` instead of `<object object at 0x7f…>`, and survival of a pickle roundtrip.  
**Where it differs from its parent:** context corruption lives in a conversation and dies with it. This lives on disk, outlives the session that introduced it, and is read by the next one as precedent — which is worse. Against that, it is visible in a diff, where a confabulated fact is not, so review can catch it; and the remedy is the same one, which is not to write it down.  
**Connections:** The mechanism is *[prose basilisks](#prose-basilisks)* one level over — memetic reproduction of a form, harmful on exposure — with code in place of prose and a repository in place of a corpus. Distinct from *the CLAUDE.md poisoning* in the field observations, which is about prose too dense for a model to finish verifying rather than about an idiom spreading.  
**Context:** Code review; why house style is worth enforcing on small things; failure modes that outlive the session that introduced them.
...
**Origin:** Einstein's objection to quantum-mechanical nonlocality — *spukhafte Fernwirkung*, from his 1947 letter to Max Born, and the informal name for the discomfort argued formally in the 🌐[EPR paper](https://en.wikipedia.org/wiki/Einstein%E2%80%93Podolsky%E2%80%93Rosen_paradox) (1935). Long since borrowed into programming, where it names an antipattern rather than a physical claim; this entry reports the programming sense.  
**Meaning:** An effect whose cause is nowhere near it: the code in front of you behaves as it does because of something written somewhere you had no reason to look. What makes it *spooky* rather than merely indirect is that the local reading is **correct and insufficient** at once — nothing at the site is wrong, and nothing at the site is enough. The reader therefore has no signal that they are missing something, which is what separates it from ordinary complexity, where the difficulty at least announces itself.  
**Connections:** In OOP the leading source is inheritance of *concrete* classes, each level piling a little behaviour onto its ancestor, so that what a call means depends on a chain the call site does not name — Julia's answer being that only abstract types may be subtyped at all, which removes the mechanism instead of warning about it. In concurrency the distance is temporal rather than lexical: the cause is not elsewhere in the text but elsewhere in *time*, on another thread. *[Concurrent ravioli](#concurrent-ravioli)* is what happens when the second kind is spread thin enough that no site shows it.  
**Context:** Used across the fleet's own conventions — per-file lint suppression is rejected on exactly this ground, being "neither a single global policy nor visible at the code site". Defining an established term in a glossary of coinages is deliberate: on any given day, some fraction of readers are meeting it for the first time, and that is a reason to explain rather than to assume (🌐[xkcd 1053](https://xkcd.com/1053/), *"one of today's lucky 10,000"*).
