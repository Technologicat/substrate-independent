...
- **Context:** Tag-line for where the term surfaces. Genre is short: a comma- or semicolon-separated list of domains, a single-noun-phrase pointer, or a brief clause plus cross-refs. Not for definitional content; not for arguments about why the concept is useful; not for case reports. The glossary is public and meant to be useful beyond the projects its terms were coined in, so name the kind of situation rather than one event from them — ticket numbers, project names and dates belong in `field-observations.md`, linked from here if the account is worth reading. An occasional specific that makes a point is fine; a Context that is only a specimen is not. The grain is the widest the evidence supports. A claim about structure — why something happens — carries to any codebase by argument, so its Context is the general domain even if every observation came from one maintainer's projects. A claim about behaviour — what a particular system tends to do — carries only as far as it was observed, so its Context names that system (cf. *Closing bell*); widening it would claim observations nobody made.
